...
$nodes = Get-ServiceFabricNode
  ...
  #declare a variable to store the values you would like to display
...
foreach($node in $nodes)
{
    ...
    $all = Get-ServiceFabricDeployedCodePackage -NodeName $node.NodeName -ApplicationName "fabric:/GettingStartedApplication" 
    foreach ($item in $all) {
    #you can add more details to the table on the activation details if you want them to get displayed
   $mytable += [pscustomobject]@{ Node=$node.NodeName; ServiceName=$item.ServiceManifestName; Status = $item.EntryPoint.EntryPointStatus; Activation=$item.EntryPoint.Statistics.LastSuccessfulActivationUtc; ProcessId=$item.EntryPoint.ProcessId}
