#The below PowerShell script helps you to delete the older blob versions based on the Last Modified date of the versions.
#This doesn't delete the Root blob/current version.
...
param(
  ...
  [Parameter(Mandatory=$true, HelpMessage="Number of days you want to keep the versions")] 
  [String] $number_of_days_old
  
)


$current_date = get-date
$date_older_for_version_to_be_deleted = $current_date.AddDays(-$number_of_days_old)
        ...
        if ($blobs -ne $null)
        {
          ...
          for ([int] $b = 0; $b -lt $blobs.Count; $b++)
          {
            ...
            if ($blobs[$b].VersionId)
            {
              ...
              if ($blobs[$b].LastModified -le $date_older_for_version_to_be_deleted)
              {
              $version_count++
              $version_usage += $blobs[$b].Length
              ##Removes all the older versions based on the number of days given
