#The below PowerShell script helps to set the access policy for a file share through calling fileshare Set Share ACL REST API.
#In this script we will construct a string to sign and generate a signature usng access key to set the access policies on a file share
...
$stringToSign = "PUT`n`n`n320`n`n"+"application/xml"+"`n`n`n`n`n`n`n"+
...
Invoke-RestMethod -Method Put -Uri $URI -Headers @{ "x-ms-date" = $date; "x-ms-version" = "2019-02-02"; "Authorization" = "SharedKey $($storageAccount):$signature" } -ContentType "application/xml" -Body $xmlBody
