#The below PowerShell script helps you to delete the older blobs based on the Last Modified date from your storage account.
...
$storage_account_key = "storage account access keys"
...
$blob_list = Get-AzureStorageBlob -Context $context -Container $container.Name -MaxCount $MaxReturn -ContinuationToken $ContinuationToken
...
foreach($blob in $blob_list){
    ...
    if($blob_date -le $date_older_for_blob_to_be_deleted) {
        ...
        # Delete the blob
        Remove-AzureStorageBlob -Container $container.Name -Blob $blob.Name -Context $context
